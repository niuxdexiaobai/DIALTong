...
public class T9Adapter extends BaseAdapter implements Filterable{
	...
	public View getView(final int position, View convertView, ViewGroup parent) {   
		...
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.home_t9_list_item, parent, false);//导入listview显示号码的布局文件
			holder = new ViewHolder();
			holder.name = (TextView) convertView.findViewById(R.id.name);  //实例化显示名字的地方
//			holder.pinyin = (TextView) convertView.findViewById(R.id.pinyin);  //实例化显示名字拼音的地方
			holder.number = (TextView) convertView.findViewById(R.id.number);  //实例化显示号码的地方
		...
		if (null == filterNum || "".equals(filterNum)) {//实现号码筛选功能
